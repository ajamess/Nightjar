...
/**
 * Synchronous hash for topic generation using Web Crypto API internally.
 * Returns a consistent hash string from input data.
 * Note: This is a simple hash for topic generation, not for security.
 */
function sha256(data) {
  const encoder = new TextEncoder();
  const bytes = encoder.encode(data);
  
  // Using a proper hash implementation based on SHA-256 constants
  // This produces deterministic 32-byte output for P2P topic discovery
  const K = [
  ...
  const padLen = ((bytes.length + 8) % 64 < 56) ? 
    56 - (bytes.length + 8) % 64 : 
    120 - (bytes.length + 8) % 64;
  const paddedLen = bytes.length + 1 + padLen + 8;
  const padded = new Uint8Array(paddedLen);
  padded.set(bytes);
  padded[bytes.length] = 0x80;
  // Append length as 64-bit big-endian
  const view = new DataView(padded.buffer);
  view.setUint32(paddedLen - 4, msgBits, false);
